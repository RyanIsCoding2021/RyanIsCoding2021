...
def add_flowers():
    ...
    if not game_over:
        new_flower()
        clock.schedule(add_flowers, 5)
    return

def check_wilt_times():
    global wilted_list, game_over, garden_happy
    if wilted_list:
        for wilted_scince in wilted_list:
            if (not wilted_scince == "happy"):
                time_wilted = int(time.time() - wilted_scince)
                if (time_wilted) > 10.0:
                    garden_happy = False
                    game_over = True
                    break
